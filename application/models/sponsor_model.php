...
class Sponsor_model extends CI_Model
{
	...
	public function get($event_id=null, $sponsor_id=null)
	{
		...
		{//if both sponsor_id and event_id exists
			//get sponsors donation with sponsor_id from an event
			$this->db->select('name, gj_join_events_sponsors.id_sponsor, id_event, value, maxvalue, qr_string');
			$this->db->from('gj_join_events_sponsors');
			$this->db->join('gj_sponsors', 'gj_sponsors.id_sponsor = gj_join_events_sponsors.id_sponsor');
			$this->db->where('gj_join_events_sponsors.id_event', $event_id);
			$this->db->where('gj_join_events_sponsors.id_sponsor', $sponsor_id);
	...
	public function updateimg($sponsor_id = null, $img_link = '')
	{
        //make array with changes
		$sponsor_data = array(
						'img_link'		=> $img_link
	...
	public function remove($id_event = 0, $id_sponsor = 0)
	{
		//deletes the event from db
		$this->db->where('id_event', $id_event);
		$this->db->where('id_sponsor', $id_sponsor);
		$this->db->delete('gj_join_events_sponsors');
	}
